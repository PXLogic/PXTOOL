...
struct context {
	...
	gboolean auto_all_logic_columns;
...
static int flush_sample_limit(const struct sr_input *in)
{
	struct context *inc;

	inc = in->priv;
	if (inc->total_samples && !inc->sample_limit_sent) {
		(void)sr_session_send_meta(in->sdi, SR_CONF_LIMIT_SAMPLES,
			g_variant_new_uint64(inc->total_samples));
		inc->sample_limit_sent = TRUE;
	}

	return SR_OK;
}

static uint8_t *pack_logic_cross_data(const struct context *inc,
	size_t *cross_len, size_t *sample_count)
{
	uint8_t *cross_data;
	...
	*cross_len = 0;
	*sample_count = 0;
	if (!inc->logic_channels || !inc->sample_unit_size)
		return NULL;

	*sample_count = inc->datafeed_buf_fill / inc->sample_unit_size;
	if (!*sample_count)
		return NULL;

	block_count = (*sample_count + 63) / 64;
	*cross_len = block_count * inc->logic_channels * 8;
	...
	for (sample_idx = 0; sample_idx < *sample_count; sample_idx++) {
		...
		sample = &inc->datafeed_buffer[sample_idx * inc->sample_unit_size];
		block = sample_idx / 64;
		bit = sample_idx % 64;
		bit_byte = bit / 8;
		bit_mask = 1 << (bit % 8);
		block_base = block * inc->logic_channels * 8;

		for (ch_idx = 0; ch_idx < inc->logic_channels; ch_idx++) {
...
static int flush_logic_samples(const struct sr_input *in)
{
	...
	if (inc->dsview_cross_data) {
		cross_data = pack_logic_cross_data(inc, &cross_len, &sample_count);
		if (!cross_data)
		...
		logic.data = cross_data;

		sr_info("CSV logic flush: rows=%zu channels=%zu sample_unit=%zu cross_bytes=%zu.",
			sample_count, inc->logic_channels, inc->sample_unit_size, cross_len);
	} else {
		cross_data = NULL;
...
static int init(struct sr_input *in, GHashTable *options)
{
	...
	inc->dsview_cross_data = g_variant_get_boolean(
		g_hash_table_lookup(options, "dsview_cross_data"));
...
static int initial_parse(const struct sr_input *in, GString *buf)
{
	...
	/*
	 * Auto-detect libsigrok's own exported logic CSV layout. These files
	 * start with a timestamp column captioned "Time(s)" followed by one
	 * logic column per enabled channel.
	 */
	if (inc->auto_all_logic_columns && inc->use_header && num_columns > 1) {
		const char *caption = columns[0] ? g_strstrip(columns[0]) : NULL;
		if (caption && g_ascii_strcasecmp(caption, "Time(s)") == 0) {
			g_free((gpointer)inc->column_formats);
			inc->column_formats = g_strdup("t,*l");
			inc->auto_all_logic_columns = FALSE;
			sr_dbg("Detected libsigrok CSV export header, using column_formats=%s.",
				inc->column_formats);
		}
	}
...
static struct sr_option options[] = {
	...
	[OPT_DSVIEW_CROSS_DATA] = {
		"dsview_cross_data", "DSView cross-data layout",
		"Internal compatibility option: emit logic packets in DSView LA_CROSS_DATA layout.",
		NULL, NULL,
	},
	[OPT_MAX] = ALL_ZERO,
};
...
static const struct sr_option *get_options(void)
{
	...
	if (!options[0].def) {
		...
		options[OPT_DSVIEW_CROSS_DATA].def = g_variant_ref_sink(g_variant_new_boolean(FALSE));
	}
