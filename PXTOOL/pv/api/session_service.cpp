...
SessionService::~SessionService() {
    if (_session) {
        _session->remove_msg_listener(this);
    }
}
...
Result<int> SessionService::configure_and_start(
    ...
    auto dbg_log = [](const char* msg) {
        ...
        if (!dbg_file.isOpen()) {
            dbg_file.setFileName(QDir::tempPath() + "/pxtool_mcp_debug.log");
    ...
    if (trigger_channel_index >= 0) {
        ...
        return Result<int>::Fail(ErrorCode::ConfigNotSupported,
                                 "Trigger preconfiguration is not supported in PXTOOL");
    ...
    if (!glitch_filters.empty()) {
        ...
        return Result<int>::Fail(ErrorCode::ConfigNotSupported,
                                 "Glitch filter setup during capture configuration is not supported in PXTOOL");
    }
    ...
    // 5c. PXTOOL manages disk-cache settings through its UI settings object.
    if (disk_cache_enabled || stream_buffer_size_gb > 0.0 ||
        stream_mem_buffer_size_gb > 0.0 || !disk_cache_path.empty()) {
        return Result<int>::Fail(ErrorCode::ConfigNotSupported,
                                 "Disk-cache and stream-buffer configuration is not supported in PXTOOL MCP");
    }
...
CaptureStatus SessionService::get_capture_status() const {
    ...
    status.is_copy_in_progress = false;
    status.is_glitch_filter_active = _session->is_glitch_filter_applied();
    status.is_signal_invert_active = false;
...
Result<uint64_t> SessionService::get_logic_samples(
    ...
    auto *snapshot = _session->get_view_logic_snapshot();
...
Result<uint64_t> SessionService::get_analog_samples(
    ...
    auto *snapshot =
        dynamic_cast<data::AnalogSnapshot*>(_session->get_snapshot(SR_CHANNEL_ANALOG));
    if (!snapshot || !snapshot->have_data())
        return Result<uint64_t>::Fail(ErrorCode::NoData,
...
Result<uint64_t> SessionService::get_dso_samples(
    ...
    auto *snapshot =
        dynamic_cast<data::DsoSnapshot*>(_session->get_snapshot(SR_CHANNEL_DSO));
    if (!snapshot || !snapshot->have_data())
        return Result<uint64_t>::Fail(ErrorCode::NoData,
...
Result<std::string> SessionService::add_decoder(
    ...
    if (!stack_on_analyzer_id.empty()) {
        return Result<std::string>::Fail(
            ...
            "Stacked decoders are not supported in PXTOOL MCP yet");
#if 0
        ...
        if (decoder_stack) {
            ...
            if (!stack.empty()) {
                ...
                // Debug: log channel mapping result
                {
                    ...
                    dsv_info("add_decoder channel mapping: %s probes=%d index_list=%d have_view_data=%d",
...
Result<std::vector<DecoderAnnotation>> SessionService::get_decoder_annotations(
    ...
    // MCP debug
    {
        static QFile s_dbg;
        if (!s_dbg.isOpen()) {
            s_dbg.setFileName(QDir::tempPath() + "/pxtool_mcp_debug.log");
...
Result<void> SessionService::set_glitch_filter(const GlitchFilterConfig &config) {
    ...
    (void)thresholds;
    (void)modes;
    return Result<void>::Fail(ErrorCode::ConfigNotSupported,
                              "Applying glitch filter from MCP is not supported in PXTOOL");
}
...
GlitchFilterConfig SessionService::get_glitch_filter_config() const {
    ...
    const auto& cfg = _session->glitch_filter_config();
    for (int i = 0; i < GLITCH_FILTER_MAX_CH; ++i) {
        if (!cfg.enabled[i] || cfg.threshold[i] == 0)
            continue;

        config.channels.push_back(i);
        config.thresholds.push_back(static_cast<int32_t>(cfg.threshold[i]));
        switch (cfg.mode[i]) {
        case data::FilterMode::HighOnly:
            config.modes.push_back(GlitchFilterMode::High);
            break;
        case data::FilterMode::LowOnly:
            config.modes.push_back(GlitchFilterMode::Low);
            break;
        case data::FilterMode::Both:
        default:
...
Result<void> SessionService::set_signal_invert(const SignalInvertConfig &config) {
    ...
    (void)config;
    return Result<void>::Fail(ErrorCode::ConfigNotSupported,
                              "Signal invert is not supported in PXTOOL");
}
...
DiskCacheInfo SessionService::get_disk_cache_info() const {
    ...
    const auto status = _session->disk_cache_status();
    info.enabled = status.enabled;
    info.write_speed_mbps = status.write_bytes_per_sec / (1024.0 * 1024.0);
    info.write_queue_depth = 0;
    info.is_disk_full = false;
...
Result<void> SessionService::save_file(const std::string &path) {
    ...
    (void)path;
    return Result<void>::Fail(ErrorCode::ConfigNotSupported,
                              "Saving to an arbitrary file path is not supported in PXTOOL MCP");
}
...
Result<void> SessionService::export_data(const ExportConfig &config) {
    ...
    (void)config;
    return Result<void>::Fail(ErrorCode::ConfigNotSupported,
                              "CSV export through StoreSession is not supported in PXTOOL MCP");
}
...
Result<void> SessionService::export_binary(const ExportConfig &config) {
    ...
    if (end == 0) {
        // Default to all data
        auto* logic = _session->get_view_logic_snapshot();
        auto* analog = _session->get_snapshot(SR_CHANNEL_ANALOG);
        auto* dso = _session->get_snapshot(SR_CHANNEL_DSO);
        if (logic && logic->have_data())
            end = logic->get_sample_count() - 1;
        else if (analog && analog->have_data())
            end = _session->cur_samplelimits() - 1;
        else if (dso && dso->have_data())
            end = _session->cur_samplelimits() - 1;
        if (end == 0)
