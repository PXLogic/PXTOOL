...
//applicationDirPath not end with '/'
    ...
    if (dir.cd("..") && dir.cd("share") && dir.cd("PXTOOL"))
    {
         return dir.absolutePath();        
    }
    QDir dir1("/usr/local/share/PXTOOL");
    if (dir1.exists()){
        return dir1.absolutePath();
    }

    dsv_err("Data directory is not exists: ../share/PXTOOL");
    assert(false);   
#else
...
QString GetFirmwareDir()
{
    ...
    QDir dir(QCoreApplication::applicationDirPath());
    // ../share/PXTOOL/res
    if (dir.cd("..") && dir.cd("share") && dir.cd("PXTOOL") && dir.cd("res"))
    {
         return dir.absolutePath();
    }
    ...
    if (dir.cd("..") && dir.cd("Resources") && dir.cd("share") && dir.cd("PXTOOL") && dir.cd("res"))
    {
         return dir.absolutePath();
    }
#endif
...
QString GetCDecodeDir()
{
    ...
    // For DreamSourceLab/PXTOOL that resolves to (at runtime, may differ per build):
